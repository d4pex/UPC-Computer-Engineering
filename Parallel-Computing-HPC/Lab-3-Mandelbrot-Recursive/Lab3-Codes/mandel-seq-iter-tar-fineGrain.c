...
void mandel_tiled(int M[ROWS][COLS], double CminR, double CminI, double CmaxR, double CmaxI, double scale_real, double scale_imag, int maxiter)
{
      ...
      for (int y = 0; y < ROWS; y += TILE) {
	    for (int x = 0; x < COLS; x += TILE) {
		 //codi1: tareador_start_task("mandelbroot");

		  equal = 1;
		  //check horizontal borders
		  for (int px = x; px < x + TILE; px++) {
			tareador_start_task("check_horizontal_borders");
			...
			tareador_end_task("check_horizontal_borders");
		  }
		  //check vertical borders
		  for (int py = y; py < y + TILE; py++) {
			tareador_start_task("check_vertical");
			...
			tareador_end_task("check_vertical");
		  }
		tareador_start_task("process_tile");
		  ...
		  if (equal) {
			...
			for (int py = y; py < y + TILE; py++) {
				...
				tareador_end_task("fill_row");
            		}
		  } else {
			// computation of tile
			for (int py = y; py < y + TILE; py++) {
				tareador_start_task("compute_row");
			      for (int px = x; px < x + TILE; px++) {
				...
				tareador_end_task("compute_row");
            		}
		  }
		tareador_end_task("process_tile");
		  //codi1: tareador_end_task("mandelbrot");
	    }
      }
}
...
int main(int argc, char *argv[])
{
      ...
      if (output2histogram)
	    histogram = calloc(maxiter, sizeof(int));
	 	tareador_disable_object(histogram); //dins de mandel_tiled utilitzem la variable per incrementar el hist[k] creant data sharing i arcs falsos a TDG
      ...
      if (output2display) {
		...
		tareador_disable_object(&scale_color); //depdencies per sharing
}
	...
	//es llegeixen dins les tasques podent generar arcs per sharing
