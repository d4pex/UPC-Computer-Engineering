...
#ifndef TILE 
#define TILE 16 /* Base case tile size */// Structure definition for complex numbers
#endif
...
void mandel_tiled_rec(int M[ROWS][COLS], int NRows, int NCols, int start_fil, int start_col, double CminR, double CminI, double CmaxR, double CmaxI, double scale_real, double scale_imag, int maxiter)
{
      ...
      if (equal) {
            ...
            tareador_start_task("fill_leaf"); // <-- CAMBIO LEAF
            ...
            tareador_end_task("fill_leaf"); // <-- CAMBIO LEAF
      } else {
            if (NCols <= TILE) {
                  // computation of tile: base case
                  tareador_start_task("compute_leaf"); // <-- CAMBIO LEAF
                  for (int py = y; py < y + NRows; py++)
                        for (int px = x; px < x + NCols; px++) {
                  ...
                  tareador_end_task("compute_leaf"); // <-- CAMBIO LEAF
            } else {
...
int main(int argc, char *argv[])
{
      ...
      if (output2histogram) {
            ...
            tareador_disable_object(&histogram); // <-- CAMBIO LEAF
      }
      ...
      tareador_disable_object(&Hmatrix); // <-- CAMBIO LEAF
