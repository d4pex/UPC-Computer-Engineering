...
#ifndef TILE
#define TILE 16           /* Base case tile size */
#endif
...
void mandel_tiled_rec(int M[ROWS][COLS], int NRows, int NCols, int start_fil, int start_col, double CminR, double CminI, double CmaxR, double CmaxI, double scale_real, double scale_imag, int maxiter)
{
      ...
      tareador_disable_object(&equal); // <-- CAMBIO TREE (disable local var)

      //check horizontal borders
      tareador_start_task("check_horiz_tree"); // <-- CAMBIO TREE
      ...
      tareador_end_task("check_horiz_tree"); // <-- CAMBIO TREE

      //check vertical borders
      tareador_start_task("check_vert_tree"); // <-- CAMBIO TREE
      ...
      tareador_end_task("check_vert_tree"); // <-- CAMBIO TREE

      //check if we can avoid computation of tile
      if (equal) {
            // fill all with the same value: base case
            tareador_start_task("fill_tree"); // <-- CAMBIO TREE
            ...
            tareador_end_task("fill_tree"); // <-- CAMBIO TREE
      } else {
            if (NCols <= TILE) {
                  // computation of tile: base case
                  tareador_start_task("compute_tree"); // <-- CAMBIO TREE
                  for (int py = y; py < y + NRows; py++)
                        for (int px = x; px < x + NCols; px++) {
...
int main(int argc, char *argv[])
{
      ...
      if (output2histogram) {
            ...
            tareador_disable_object(&histogram); // <-- CAMBIO TREE
      }
      ...
      tareador_disable_object(&Hmatrix); // <-- CAMBIO TREE
