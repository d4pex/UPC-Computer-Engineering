...
void mandel_simple(int M[ROWS][COLS], double CminR, double CminI, double CmaxR, double CmaxI, double scale_real, double scale_imag, int maxiter)
{
      #pragma omp parallel
      {
      	      ...
      	      int x_start = my_id * (COLS / num_threads);
      	      int x_end = (my_id + 1) * (COLS / num_threads);
	      ...
	      for (int y = 0; y < ROWS; y++) {
		    for (int x = x_start; x < x_end; x++) {
			  M[y][x] = pixel_dwell(COLS, ROWS, CminR, CminI, CmaxR, CmaxI, x, y, scale_real, scale_imag, maxiter);
			  if (output2histogram)
			  	#pragma omp atomic
				histogram[M[y][x] - 1]++;
			  if (output2display) {
