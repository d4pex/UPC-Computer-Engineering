...
void mandel_simple(int M[ROWS][COLS], double CminR, double CminI, double CmaxR, double CmaxI, double scale_real, double scale_imag, int maxiter)
{
      #pragma omp parallel
      {
	      ...
	      for (int y = 0; y < ROWS; y++) {
		    for (int jj = my_id * BS; jj < COLS; jj += num_threads * BS) {
		    	int LIMIT = (jj + BS < COLS) ? (jj + BS) : COLS;
		    	for (int j = jj; j < LIMIT; j++) {
				  M[y][j] = pixel_dwell(COLS, ROWS, CminR, CminI, CmaxR, CmaxI, j, y, scale_real, scale_imag, maxiter);
				  if (output2histogram)
				  	#pragma omp atomic
					histogram[M[y][j] - 1]++;
				  if (output2display) {
					/* Scale color and display point  */
					long color = (long)((M[y][j] - 1) * scale_color) + min_color;
					#pragma omp critical
					{
						if (setup_return == EXIT_SUCCESS) {
						      XSetForeground(display, gc, color);
						      XDrawPoint(display, win, gc, j, y);
						}
					}
				  }
			}
