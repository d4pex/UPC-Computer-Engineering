...
#ifndef ROWS
#define ROWS (128*8)             /* size of display window in pixels */
#endif

#ifndef COLS
#define COLS (128*8)             /* size of display window in pixels */
#endif
...
void mandel_tiled(int M[ROWS][COLS],
                  double CminR, double CminI,
                  double CmaxR, double CmaxI,
                  double scale_real, double scale_imag,
                  int maxiter)
{
    #pragma omp parallel
    {
            ...
            for (int y = 0; y < ROWS; y += TILE) {
                for (int x = 0; x < COLS; x += TILE) {
                    ...
                    #pragma omp task 
                    {
                        ...
                        /* ============================
                         *  FASE 1: comprovar vores
                         * ============================ */

                        /* vores verticals */
                        for (int py = y; py < y + TILE; py++) {
                            /* columna esquerra */
                            M[py][x] = pixel_dwell(COLS, ROWS,
                                                   CminR, CminI, CmaxR, CmaxI,
                                                   x, py,
                                                   scale_real, scale_imag, maxiter);
                            /* columna dreta */
                            M[py][x + TILE - 1] = pixel_dwell(COLS, ROWS,
                                                              CminR, CminI, CmaxR, CmaxI,
                                                              x + TILE - 1, py,
                                                              scale_real, scale_imag, maxiter);
                        ...
                        for (int px = x; px < x + TILE; px++) {
                            /* fila de dalt */
                            M[y][px] = pixel_dwell(COLS, ROWS,
                                                   CminR, CminI, CmaxR, CmaxI,
                                                   px, y,
                                                   scale_real, scale_imag, maxiter);
                            /* fila de baix */
                            M[y + TILE - 1][px] = pixel_dwell(COLS, ROWS,
                                                              CminR, CminI, CmaxR, CmaxI,
                                                              px, y + TILE - 1,
                                                              scale_real, scale_imag, maxiter);
                        ...
                        /* ============================
                         *  FASE 2: omplir o computar tile
                         * ============================ */

                        if (equal) {
                            /* tile uniforme: tots els punts tenen el mateix dwell */
                            int dwell = M[y][x];
                            long color = 0;

                            if (output2display) {
                                color = (long)((dwell - 1) * scale_color) + min_color;
                            }

                            if (output2histogram) {
                                int idx = dwell - 1;
                                #pragma omp atomic
                                histogram[idx] += TILE * TILE;
                            }
                            ...
                            /* tile no uniforme: cal calcular tots els píxels */
                            for (int py = y; py < y + TILE; py++) {
                                for (int px = x; px < x + TILE; px++) {
                                    int dwell = pixel_dwell(COLS, ROWS,
                                                            CminR, CminI, CmaxR, CmaxI,
                                                            px, py,
                                                            scale_real, scale_imag, maxiter);
                                    M[py][px] = dwell;
                                    ...
                                    if (output2display) {
                                        long color = (long)((dwell - 1) * scale_color) + min_color;
                                        #pragma omp critical
                                        {
                                            if (setup_return == EXIT_SUCCESS) {
                                                XSetForeground(display, gc, color);
...
int main(int argc, char *argv[])
{
      ...
      int height = ROWS;
      double size = N, x0 = -1.82541, y0= -0.204126;
      int *Hmatrix;
      unsigned char *image;
      TRACE_START_MARK;
