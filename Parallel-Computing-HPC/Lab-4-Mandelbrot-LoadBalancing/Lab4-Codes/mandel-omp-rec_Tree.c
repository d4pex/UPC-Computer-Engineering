...
#ifndef ROWS
#define ROWS (8*128)             /* size of display window in pixels */
#endif

#ifndef COLS
#define COLS (8*128)             /* size of display window in pixels */
#endif
...
void mandel_tiled_rec(int M[ROWS][COLS], int NRows, int NCols, int start_fil, int start_col, double CminR, double CminI, double CmaxR, double CmaxI, double scale_real, double scale_imag, int maxiter)
{
      ...
      x = start_col;
		...
		int equal1=non_first_call, equal2=non_first_call;
		...
		#pragma omp task shared (equal1) 
      		for (int py = y; py < y + NRows; py++) {
	    		...
	    		equal1 = equal1 & (M[y][x] == M[py][x + NCols - 1]);
      		}
		#pragma omp task shared (equal2)
      		for (int px = x; px < x + NCols; px++) {
	    		...
	    		equal2 = equal2 & (M[y][x] == M[y + NRows - 1][px]);
      		}
		#pragma omp taskwait
		equal = equal1&equal2;
